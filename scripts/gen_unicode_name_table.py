...
OUT_H = Path("unicode_name_table.h")
OUT_C = Path("unicode_name_table.c")
...
def emit_u8_array(f, ctype: str, name: str, values: list[int], per_line: int = 16) -> None:
...
def generate() -> None:
    ...
    with OUT_H.open("w", encoding="ascii") as f:
        f.write("/* Generated by scripts/gen_unicode_name_table.py - do not edit. */\n")
        f.write("\n")
        f.write("#pragma once\n")
        f.write("\n")
        ...
        f.write(
            f"extern const uint8_t unicode_name_block_first_names[{len(first_blob)}];\n"
        )
        f.write(
            f"extern const uint32_t unicode_name_block_data_offsets[{block_count + 1}];\n"
        )
        f.write(
            f"extern const uint8_t unicode_name_block_data[{len(block_data)}];\n"
        )

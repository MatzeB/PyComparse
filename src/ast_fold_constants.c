  ...
  case AST_UNEXPR_STAR_STAR:
    ...
    if (type == AST_UNEXPR_NOT) {
      ...
      if (ast_expression_type(inner) == AST_COMPARISON
        ...
        uint8_t negated = op == COMPARE_OP_IS       ? COMPARE_OP_IS_NOT
                          : op == COMPARE_OP_IS_NOT ? COMPARE_OP_IS
                          : op == COMPARE_OP_IN     ? COMPARE_OP_NOT_IN
                          : op == COMPARE_OP_NOT_IN ? COMPARE_OP_IN
                                                    : 0;
        if (negated) {
          inner->comparison.operands[0].op = negated;
          return inner;
        }
      }
    }

...
def pack_block(values):
    ...
    for i in range(0, BLOCK_SIZE, 4):
        byte = values[i] | (values[i + 1] << 2) | (values[i + 2] << 4) | (values[i + 3] << 6)
        packed.append(byte)
    return bytes(packed)
...
def generate():
    ...
    print(f"Blocks: {num_blocks} total, {num_unique} unique", file=sys.stderr)
    print(f"index1: {len(index1)} bytes", file=sys.stderr)
    print(f"index2: {num_unique * bytes_per_block} bytes", file=sys.stderr)
    print(f"Total:  {len(index1) + num_unique * bytes_per_block} bytes", file=sys.stderr)
    ...
    with open("unicode_xid_table.c", "w") as f:
        ...
        p("/* Generated by scripts/gen_xid_table.py — do not edit. */")
        p()
        p('#include "unicode_xid_table.h"')
        p()
        p(f"const uint8_t xid_index1[{len(index1)}] = {{")
        ...
        p(f"const uint8_t xid_index2[{total_bytes}] = {{")
    ...
    with open("unicode_xid_table.h", "w") as f:
        p = lambda *a, **kw: print(*a, file=f, **kw)
        p("/* Generated by scripts/gen_xid_table.py — do not edit. */")
        p("/* Unicode XID_Start / XID_Continue lookup table (Unicode 12.1.0) */")
        p()
        p("#pragma once")
        p()
        p("#include <stdint.h>")
        p()
        p(f"#define XID_SHIFT {SHIFT}")
        p(f"#define XID_BLOCK_MASK {hex(BLOCK_SIZE - 1)}")
        p()
        p(f"extern const uint8_t xid_index1[{len(index1)}];")
        p(f"extern const uint8_t xid_index2[{total_bytes}];")
        p()
        p("static inline int is_xid_continue(uint32_t cp)")
        p("{")
        p(f"  if (cp >= 0x{MAX_CP:x}) return 0;")
        p(f"  uint32_t block = xid_index1[cp >> XID_SHIFT];")
        p(f"  uint32_t offset = cp & XID_BLOCK_MASK;")
        p(f"  uint8_t packed = xid_index2[block * {bytes_per_block} + (offset >> 2)];")
        p(f"  return (packed >> ((offset & 3) * 2)) & 2;")
        p("}")
        p()
        p("static inline int is_xid_start(uint32_t cp)")
        p("{")
        p(f"  if (cp >= 0x{MAX_CP:x}) return 0;")
        p(f"  uint32_t block = xid_index1[cp >> XID_SHIFT];")
        p(f"  uint32_t offset = cp & XID_BLOCK_MASK;")
        p(f"  uint8_t packed = xid_index2[block * {bytes_per_block} + (offset >> 2)];")
        p(f"  return (packed >> ((offset & 3) * 2)) & 1;")
        p("}")
